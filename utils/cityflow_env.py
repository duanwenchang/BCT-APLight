...
class Intersection:
    ...
    def update_current_measurements(self, simulator_state):
        ...
        list_vehicle_new_arrive = list(
            set(self.list_lane_vehicle_current_step_in) - set(self.list_lane_vehicle_previous_step_in))
    ...
    def _update_feature(self):
        ...
        dic_feature["lane_num_vehicle_upstream"] = self._get_lane_num_vehicle_entring()
        dic_feature["lane_num_vehicle_next_upstream"] = self._get_lane_num_vehicle_downstream()
        ...
        dic_feature["downstream_pressure"] = self._get_traffic_movement_pressure_efficient(
            dic_feature["lane_num_waiting_vehicle_in"], dic_feature["lane_num_waiting_vehicle_out"])


        tmp_part_n, tmp_part_q, tmp_efficient_part, enter_running_part, lepq, exit_running_part, lepq2 = self._get_part_traffic_movement_features()

        dic_feature["lane_enter_running_part"] = list(enter_running_part)
        dic_feature["lane_exit_running_part"] = list(exit_running_part)
    ...
    def _get_part_traffic_movement_features(self):
        ...
        part_exiting_running = np.array(list_exiting_num_l) - np.array(list_exiting_part_queue)

        return traffic_movement_pressure_nums, tmp_queue_part, tmp_queue_efficient_part, part_entering_running, list_entering_part_queue, part_exiting_running, list_exiting_part_queue
