    ...
    def make_reward(self, folder, i):
        ...
        try:
            ...
            for time in range(0, total_time - self.measure_time + 1, self.interval):
                state_list = self.construct_state(self.dic_traffic_env_conf["LIST_STATE_FEATURE"], time, i)
                # print(state_list)
                ...
                if time + self.interval == total_time:
                    next_state_list = self.construct_state(self.dic_traffic_env_conf["LIST_STATE_FEATURE"],
                                                      time + self.interval - 1, i)
                else:
                    next_state_list = self.construct_state(self.dic_traffic_env_conf["LIST_STATE_FEATURE"],
                                                      time + self.interval, i)

                q_values = self.logging_data_list_per_gen[i][time].get('q_values')

                sample = [state_list, action, next_state_list, reward_average, reward_instant, q_values,time,
