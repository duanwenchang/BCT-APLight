...
class Updater:
    ...
    def load_sample_for_agents(self):
        ...
        if self.dic_traffic_env_conf['MODEL_NAME'] in ["EfficientPressLightOne", "PressLightOne", "MPLight"]:
            self.sample_set_all = []
            for i in range(self.dic_traffic_env_conf['NUM_INTERSECTIONS']):
                sample_set = self.load_sample_with_forget(i)
                self.sample_set_all.extend(sample_set)
            self.agents[0].prepare_Xs_Y(self.sample_set_all)
            
        elif self.dic_traffic_env_conf['MODEL_NAME'] in ["PressLight", "EfficientPressLight"]:
            for i in range(self.dic_traffic_env_conf['NUM_INTERSECTIONS']):
                sample_set = self.load_sample_with_forget(i)
                self.agents[i].prepare_Xs_Y(sample_set)
                
        elif self.dic_traffic_env_conf['MODEL_NAME'] == "BCT-AT":
            self.samples_list = []
            for i in range(self.dic_traffic_env_conf['NUM_INTERSECTIONS']):
                sample_set = self.load_sample_with_forget(i)
                self.samples_list.append(sample_set)
            self.agents[0].prepare_Xs_Y(self.samples_list)
            
        elif self.dic_traffic_env_conf['MODEL_NAME'] == "Colight":
            self.samples_list = []
            for i in range(self.dic_traffic_env_conf['NUM_INTERSECTIONS']):
                sample_set = self.load_sample_with_forget(i)
                self.samples_list.append(sample_set)
    ...
    def update_network(self, i):
        ...
        # # self.agents[i].train_predict_network(
        #     os.path.join(self.dic_path["PATH_TO_WORK_DIRECTORY"], "train_round"))
        self.agents[i].save_network("round_{0}_inter_{1}".format(self.cnt_round, self.agents[i].intersection_id))
        # self.agents[i].save_predict_network("round_{0}_inter_{1}_predict".format(self.cnt_round, self.agents[i].intersection_id))
